...
function download_files(){
    ...
    if [ $? -eq 0 ];then
        ...
        echo ""
        echo "Unzip shadowsocks-libev failed! Please visit http://ipv.pw/ and contact."
        exit 1
    fi
    # Download start script
    if ! wget --no-check-certificate https://raw.githubusercontent.com/CUBEGWZ/Shadowsocks/master/Libev/shadowsocks-libev; then
...
function install(){
    ...
    if [ -s /usr/local/bin/ss-server ];then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo ""
            echo "Shadowsocks-libev install failed! Please visit http://ipv.pw/ and contact."
            exit 1
        fi
    fi
    cd $cur_dir
