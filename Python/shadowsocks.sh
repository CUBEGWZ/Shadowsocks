...
echo "# Intro: http://ipv.pw/                                     #"
echo "# Author: CUBE <me@diip.me>                                 #"
echo "#############################################################"
echo ""
...
function pre_install(){
    ...
    read -p "(Default port: 8585):" shadowsocksport
    [ -z "$shadowsocksport" ] && shadowsocksport="8585"
...
function download_files(){
    if [ "$OS" == 'CentOS' ]; then
        ...
        if ! wget --no-check-certificate https://raw.githubusercontent.com/CUBEGWZ/Shadowsocks/master/Python/shadowsocks -O /etc/init.d/shadowsocks; then
            echo "Failed to download shadowsocks chkconfig file!"
            exit 1
        fi
    else
        if ! wget --no-check-certificate https://raw.githubusercontent.com/CUBEGWZ/Shadowsocks/master/Python/shadowsocks-debian -O /etc/init.d/shadowsocks; then
            echo "Failed to download shadowsocks chkconfig file!"
            exit 1
        fi
    fi
...
function install_ss(){
    ...
    if [ -f /usr/bin/pip ]; then
        ...
        if [ -f /usr/bin/ssserver ] || [ -f /usr/local/bin/ssserver ]; then
            ...
            echo ""
            echo "Shadowsocks install failed! Please visit http://ipv.pw/ and contact."
            exit 1
        fi
        clear
        echo ""
        ...
        echo ""
        echo "Welcome to visit:http://ipv.pw/"
        echo ""
        exit 0
    else
        echo ""
        echo "pip install failed! Please visit http://ipv.pw/ and contact."
        exit 1
    fi
}
